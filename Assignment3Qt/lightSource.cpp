...
AreaLight::AreaLight(float w, float h, glm::vec3 pos, glm::vec3 totalColor, glm::vec3 dRight, glm::vec3 dDown)
	...
	, h(h)
{
	...
	float numF = totalColor.x / UNITCOLORINTENSITY;
	numF = glm::max(numF, totalColor.y / UNITCOLORINTENSITY);
	numF = glm::max(numF, totalColor.z / UNITCOLORINTENSITY);
	int num = static_cast<int>(numF);

	std::vector<glm::vec2> point;
	BestCandidateAlgorithm(point, num, w, h);

	for (int i = 0; i < num; i++)
	{
		glm::vec3 pointPos;
		pointPos = pos + dRight * point[i].x + dDown * point[i].y;
		pointSamples.push_back(new PointLight(pointPos, totalColor / (float)num));
	}
}
AreaLight::~AreaLight()
{
	...
	, quadT(NULL)
{
	...
	for (unsigned int i = 0; i < quadT->areaColor.size(); i++)
	{
		...
		this->lightSamples.push_back(new AreaLight(quadT->sizeWH[i][0], quadT->sizeWH[i][1], pos, quadT->areaColor[i], dRight, dDown));
	}
}
SquareMap::~SquareMap()
{
...
void SquareMap::RayIntersection(RayClass* ray, RayHitObjectRecord &rhor)
{
	...
	while (t > EPSILON)
	{
		...
		//glm::vec3 e1 = this->dDown;
		//glm::vec3 n1 = cross(this->normal, e1);
		//glm::vec3 n1_ = hitPoint - ulCorner;
		//if (dot(n1, n1_) < 0)
		//	break;

		//glm::vec3 e2 = this->dRight;
		//glm::vec3 n2 = cross(this->normal, e2);
		//glm::vec3 n2_ = hitPoint - ulCorner - dDown * size;

		//glm::vec3 e3 = -this->dDown;
		//glm::vec3 n3 = cross(this->normal, e3);
		//glm::vec3 n3_ = hitPoint - ulCorner - dDown * size - dRight * size;
		//if (dot(n3, n3_) < 0)
		//	break;

		//glm::vec3 e4 = -this->dRight;
		//glm::vec3 n4 = cross(this->normal, e4);
		//glm::vec3 n4_ = hitPoint - ulCorner - dRight * size;
		//if (dot(n4, n4_) < 0)
		//	break;
